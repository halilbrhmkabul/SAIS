...
public class Muracaat extends BaseEntity {
    ...
    @Id
    @SequenceGenerator(name = "muracaat_seq", sequenceName = "muracaat_seq", allocationSize = 1)
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "muracaat_seq")
    private Long id;
    ...
    @JoinColumn(name = "basvuru_sahibi_id", nullable = false, foreignKey = @ForeignKey(ConstraintMode.NO_CONSTRAINT))
    ...
    @Column(name = "kendisi_basvurdu", nullable = false, columnDefinition = "NUMBER(1,0) DEFAULT 1")
    @Builder.Default
    private Boolean kendisiBasvurdu = true;

    @ManyToOne(fetch = FetchType.EAGER)
    @JoinColumn(name = "adina_basvurulan_kisi_id", foreignKey = @ForeignKey(ConstraintMode.NO_CONSTRAINT))
    ...
    @Column(name = "basvuru_metni", columnDefinition = "CLOB")
    private String basvuruMetnu;

    @Column(name = "personel_gorus_notu", columnDefinition = "CLOB")
    private String personelGorusNotu;

    @Column(name = "dokuman_listesi", columnDefinition = "CLOB")
    ...
    @ManyToOne(fetch = FetchType.EAGER)
    @JoinColumn(name = "kaydeden_personel_id", foreignKey = @ForeignKey(ConstraintMode.NO_CONSTRAINT))
