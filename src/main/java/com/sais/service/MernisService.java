...
public class MernisService {
    ...
    public MernisService(ValidationUtil validationUtil) {
    ...
    private MernisKisiDTO simulateKisiSorgusu(String tcKimlikNo) {
        log.info("MERNİS Simülasyon - MERNİS servisine bağlanıldı, kişi bilgileri sorgulanıyor...");
        ...
        MernisKisiDTO kisi = MernisKisiDTO.builder()
                ...
                .email(ad.toLowerCase() + "." + soyad.toLowerCase() + "@example.com")
                .build();
        ...
        if (checkGebzeAddress && !ValidationUtil.isGebzeAddress(kisi.getIl(), kisi.getIlce())) {
            log.warn("Kişi Gebze dışında ikamet ediyor: " + kisi.getIl() + "/" + kisi.getIlce());
        } else {
            log.info("MERNİS'ten kişi bilgileri başarıyla çekildi: " + kisi.getAd() + " " + kisi.getSoyad());
        }
    ...
    private List<MernisYakinDTO> simulateYakinlikSorgusu(String tcKimlikNo) {
        log.info("MERNİS Yakınlık Servisi Simülasyon - Yakınlık servisi sorgulanıyor...");
        ...
        log.info("Yakınlık servisi sorgulandı: " + yakinlar.size() + " yakın bulundu");
