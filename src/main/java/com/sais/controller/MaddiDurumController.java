...
public class MaddiDurumController implements Serializable {
    ...
    public void saveMaddiDurum() {
        try {
            ...
            MessageUtil.showInfoMessage("Başarılı", "Maddi durum bilgileri kaydedildi");
    ...
    public void saveMaddiDurumVeIleri() {
        try {
            ...
            if (muracaatId != null) {
                try {
                    muracaatWorkflowService.sevkTahkikata(muracaatId);
                    muracaatWorkflowService.markMaddiDurumCompleted(muracaatId);
                    
                    log.info("✓ Maddi durum kaydedildi ve müracaat tahkikata sevk edildi: {}", muracaatId);
                    
                    MessageUtil.showInfoMessage("Başarılı", "Maddi durum kaydedildi. Müracaat tahkikata sevk edildi.");
                    
                } catch (Exception e) {
                    log.error("Tahkikata sevk hatası", e);
                    MessageUtil.showErrorMessage("Hata", "Tahkikata sevk edilemedi: " + e.getMessage());
                }
            }
